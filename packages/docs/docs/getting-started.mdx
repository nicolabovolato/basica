...
We then import AppBuilder and start registering sample, our service both as a healthcheck and an entrypoint.
...
<Tabs groupId="ts-style">
  <TabItem value="classes" label="Classes">
    ...
    const app = new AppBuilder(container)
      .configureHealthchecks(b => b
        .addHealthcheck("sample", (c) => c.sample)
      )
  ...
  <TabItem value="classes" label="Classes">
    ...
    const app = new AppBuilder(container)
      .configureHealthchecks(b =>
        b.addHealthcheck("sample", (c) => c.sample)
      )
      .configureLifecycle((b, c) => b
        .with(lifecyclePlugin, (b)
