...
:::note
Healthchecks/services/entrypoints registered through a plugin are typed on the App as long as
the plugin advertises its registrations (all official plugins do, see [Writing plugins](../plugins/writing-plugins#typing-what-a-plugin-registers) for more info).
...
export const getApp = (provider: ConfigProvider = envProvider()) => {
    ...
    // ...
    return AppBuilder.registerDependencies()
        .configureLifecycle((b) => b
...
type _ = typeof app.entrypoints.http // FastifyEntrypoint
const { fastify } = app.entrypoints.http
