...
The examples below use [Zod](https://zod.dev). When reading from environment variables, the schema
...
type declared in the schema, so plain `z.number()` / `z.boolean()` work — no `z.coerce` needed.
:::
...
```ts title="json file provider"
import { readFileSync } from "node:fs";
import { ConfigProvider } from "@basica/config";

const jsonFileProvider = (path: string): ConfigProvider => ({
