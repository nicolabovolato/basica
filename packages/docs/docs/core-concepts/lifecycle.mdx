...
Once the lifecycle is configured, `app.run()` hands the process over to Basica:
...
Because `run()` owns `process.exit` and the signal handlers, it's **not** meant for tests, or for embedding an app inside a larger process. For those, drive the lifecycle yourself:
