import { AppRequiredDeps, IHealthcheckManager } from "@basica/core";
...
export class FastifyEntrypointBuilder<D extends AppRequiredDeps> {
  ...
  mapHealthchecks(
    ...
    // healthcheck HTTP response contract: a result serialized with its name,
    // minus the internal `error` field
    const status = z.enum(["healthy", "unhealthy"]);
