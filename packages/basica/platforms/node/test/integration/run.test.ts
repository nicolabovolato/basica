...
// The vitest process always runs on Node; it spawns the fixture under the
// runtime being verified (set via RUNTIME; defaults to node). This is how the
// same `run()` behaviour is proven across the runtimes this package supports.
const RUNTIME = process.env.RUNTIME ?? "node";
const argv: Record<string, string[]> = {
  // node strips the .ts fixture's types at runtime; silence the "typeless
  // package" notice it prints for an ESM .ts under a CommonJS package.
  node: ["node", "--disable-warning=MODULE_TYPELESS_PACKAGE_JSON"],
  bun: ["bun"],
};
const [bin, ...flags] = argv[RUNTIME] ?? argv.node;
